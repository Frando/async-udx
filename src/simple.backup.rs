...
impl UdxStreamInner {
    ...
    fn poll_drive_inner(&mut self, cx: &mut Context<'_>) -> io::Result<bool> {
        ...
        should_continue |= self.poll_incoming(cx);
    ...
    fn poll_transmit(&mut self, _cx: &mut Context<'_>) -> bool {
        let mut iters = 0;
    ...
    fn poll_incoming(&mut self, cx: &mut Context<'_>) -> bool {
    ...
    fn handle_remote_ack(&mut self, seq: u32) {
        if seq <= self.remote_acked {
            return;
        }
    ...
    // fn pending_sends_reached(&self) -> bool {
    //     self.seq.checked_sub(self.remote_acked).unwrap_or(0u32) > MAX_PENDING
    // }
}
